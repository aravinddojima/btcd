...
With Docker you can easily set up *btcd* to run your Bitcoin full node. You can find the official *btcd* Docker images on Docker Hub [btcsuite/btcd](https://hub.docker.com/r/btcsuite/btcd). The Docker source file of this image is located at [Dockerfile](https://github.com/aravinddojima/btcd/blob/master/Dockerfile).
...
services:
  btcd:
    container_name: btcd
    hostname: btcd
    build: https://github.com/aravinddojima/btcd.git#master
    restart: unless-stopped
    volumes:
      - btcd-data:/root/.btcd
    ports:
